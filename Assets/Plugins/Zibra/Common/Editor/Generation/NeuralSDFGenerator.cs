    ...
    ***REMOVED***
    ...
    internal abstract class NeuralSDFGenerator
    {
        ...
        protected bool CheckMeshSize()
        {
            ...
            {
                string errorMessage =
                    $"Mesh is too large. Can't generate representation. Triangle count should not exceed {REQUEST_TRIANGLE_COUNT_LIMIT***REMOVED*** triangles, but current mesh have {MeshToProcess.triangles.Length / 3***REMOVED*** triangles";
        ...
        ***REMOVED***
        ...
        protected void SendRequest(string json)
        {
            ...
            ***REMOVED***
            ...
            {
                string errorMessage =
                    $"Mesh is too large. Can't generate representation. Please decrease vertex/triangle count. Web request should not exceed {REQUEST_SIZE_LIMIT / (1 << 20):N2***REMOVED***mb, but for current mesh {(float)json.Length / (1 << 20):N2***REMOVED***mb is needed.";
                EditorUtility.DisplayDialog("Zibra Effects Error.", errorMessage, "OK");
                Debug.LogError(errorMessage);
                return;
            ***REMOVED***
            ...
            ***REMOVED***
            ...
            var meshRepresentation =
                new MeshRepresentation { vertices = MeshToProcess.vertices.Vector3ToString(),
                                         ...
                                         cutoff_weight = 0.1f,
                                         static_quantization = true ***REMOVED***;
            ...
            for (int i = 0; i < sharedMesh.vertexCount; i++)
            {
                if (i % 100 == 0)
                {
                    EditorUtility.DisplayProgressBar(
                        ...
                        $"Starting Generation: Processing vertices {i***REMOVED***/{sharedMesh.vertexCount***REMOVED***",
    ...
    internal class ProgressManager
    {
        ...
        ***REMOVED***
        ...
        public void FinishTask()
        {
            Completed++;
            Remaining--;
            Progress.Report(ID, (float)(Completed) / (Completed + Remaining), $"{Completed***REMOVED*** / {Completed + Remaining***REMOVED***");
        ...
        ***REMOVED***
        ...
        public void AddTask()
        {
            ...
            if (ID == 0)
            {
                ID = Progress.Start(Message, $"0 / {Completed + Remaining***REMOVED***");
                Progress.RegisterCancelCallback(ID, Cancel);
            ***REMOVED***
                
            Progress.Report(ID, Completed / (Completed + Remaining), $"{Completed***REMOVED*** / {Completed + Remaining***REMOVED***");
        ***REMOVED***
